...
class trainDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        x, x_size, y = self.data_generation(self.x_tr, self.y_tr, self.inds_shuffle, index)
        return (x, x_size), y
    ...
    def data_generation_val(self):
        ...
        x_val, x_size_val, y_val = self.data_generation(self.x_val, self.y_val, self.inds, -1)
        return x_val, x_size_val, y_val

    def __len__(self):
        # number of batches in one epoch
        batch_num = int(self.tr_num/self.batch_size)
...
class testDataGenerator(tf.keras.utils.Sequence):
    def __init__(self, year=2017, split=0, cand_num=4):
        self.data_path = f"data/{year}-{year}-split{split}"
...
if __name__ == '__main__':   
    ...
    test_example_path = f"json_data/pickles_plus/test_examples_ncomb_1_ncands_{n_cands}"
    ...
    make_packed_example_test_pickle(test_example_files, f"{data_path}/test5_example_cand{n_cands}.pkl", max_item_num=max_item_num)
#-------------------------------
